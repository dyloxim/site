...
// const { getTraceEvents } = require("next/dist/trace");
...
const zip = (list1, list2) => {
    return list1.map((value, key) => [value, list2[key]]);
}
const getComposition = (...functions) => {
  return (point) => {
        return functions.reduce((value, func) => func(value), point);
    };
}
const override = (defaults, revision) => {
  let returnValues = {};
  if (!revision) {
    ...
    for (index in Object.keys(defaults)) {
      let key = Object.keys(defaults)[index];
      returnValues[key] =  
        (revision[key]) ? revision[key] : defaults[key];
    }
    return returnValues;
  }
}
...
const mProduct = (...matricies) => {
  matricies.reduce((A, B) => [getLinear(A)(B[0]), getLinear(A)(B[1])]);
}
const getAffine = (M,c) => getComposition(getLinear(M), getTranslation(c));
const getLinear = M => p => {
  return [
    p[0] * M[0][0] + p[1] * M[1][0],
    p[0] * M[0][1] + p[1] * M[1][1]
...
const randomAffine = (rotation, uniform) => {
  ...
  // if (uniform) { r[3] = r[0]; }
  let newRandomLinear = randomLinear()
  let t1 = [0,0].map(() => Math.random() * 1.5 - .75);
  return getComposition(newRandomLinear, getTranslation(t1));
}
const randomLinear = () => {
  let b1 = [0,0].map(() => Math.random() * 2 - 1);
  while(dot(b1,b1) <= .2) {
    b1 = [0,0].map(() => Math.random() * 2 - 1);
  }
  let b2 = [0,0].map(() => Math.random() * 2 - 1);
  while(dot(b1, b2)/dot(b1,b1) < .4) {
    b2 = [0,0].map(() => Math.random() * 2 - 1);
  }
  return getLinear([b1,b2]);
}
...
const getOutputRegion = (referenceRegion, canvas, options) => {
  let outputRegion = { o: [-1,-1], x: [2,0], y: [0,2] };
...
const drawLine = ([x1,y1], [x2,y2], imgData, rgb) => {
  ...
  if (steep) {
    for (var x = xpxl1 + 1; x < xpxl2; x++) {
      paintPixel([ipart(intery), x],     imgData, rgb.concat(255));
      intery = intery + m;
    }
  } else {
    for (var x = xpxl1 + 1; x < xpxl2; x++) {
      paintPixel([x, ipart(intery)],     imgData, rgb.concat(255));
      intery = intery + m;
    }
  }
}
...
const inferScalingFactor = M => {
  let e1 = vMinus(M([1,0]), M([0,0]));
  let e2 = vMinus(M([0,1]), M([0,0]));
...
const IFS = {
  init: (def, options) => {
    ...
    if (def.probabilities === 'equal') {
      ...
      let scales = def.transforms
          .map(M => inferScalingFactor(M))
  ...
  setReport: options => {
    ...
    let latexTransforms = IFS.properties.transforms.map((v,k) => transformToLatex(k, v, color));
    latexTransforms = latexTransforms.reduce((prev, current) => prev + "\\\\" + current);
    IFS.latex = "$$\\begin{align}" + latexTransforms + "\\end{align}$$";
  },
}
...
const koch = {
  ...
  transforms: [
    getLinear([[1/3, 0],[0,1/3]]),
    getLinear([[1/2, 0.866025],[-0.866025,1/2]]),
    getTranslation([1/Math.sqrt(2), 2]),
  ],
  probabilities: Array(3).fill(1/3),
};
const random = {
